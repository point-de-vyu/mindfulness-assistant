...
def get_categories(
    sos_mng: SosMngDep, user_id: int = Depends(authentication)
) -> List[str]:
...
def get_default_sos_rituals(
    ...
    try:
        user_rituals = sos_mng.get_default_rituals(
            category_name=category, situation_name=situation
        )
...
def get_ritual_by_id(
    ritual_id: int, sos_mng: SosMngDep, user_id: int = Depends(authentication)
) -> SosRitual:
...
@router.post("/custom_sos_ritual/", summary="Add a custom ritual created by a user")
...
@router.post("/default_sos_ritual/", summary="Add a default ritual to user's own")
...
@router.delete(
    "/sos_rituals/{ritual_id}", summary="Delete a chosen ritual from a user's data"
)
...
@router.post("/sos_feedback/", summary="Log a journal entry about a ritual")
