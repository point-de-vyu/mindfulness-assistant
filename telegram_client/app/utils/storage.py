...
async def get_storage_key(user_id: int) -> StorageKey:
    return StorageKey(
        bot_id=int(os.environ["BOT_ID"]), chat_id=user_id, user_id=user_id
    )


async def add_data_to_storage(
    data: Dict[str, Any], user_id: int, storage: BaseStorage
) -> None:
...
async def extract_data_from_storage(
    key: str, user_id: int, storage: BaseStorage
) -> Any:
    storage_key = await get_storage_key(user_id)
