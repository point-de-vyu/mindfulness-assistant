...
from telegram_client.app.utils.storage import add_data_to_storage, delete_key_from_storage
...
async def get_journal_entry_for_ritual(callback: CallbackQuery, state: FSMContext):
    ...
    await add_data_to_storage({MemoryKey.CURRENT_RITUAL: ritual_id}, callback.from_user.id, state.storage)
...
@router.message(F.text.lower().in_({"cancel", "quit", "exit", "no", "nah"}), Journaling.is_journaling)
async def refused_journaling(message: Message, state: FSMContext):
    await delete_key_from_storage(MemoryKey.CURRENT_RITUAL, message.from_user.id, state.storage)
...
async def save_journal_entry(message: Message, state: FSMContext):
    ...
    ritual_id = await delete_key_from_storage(MemoryKey.CURRENT_RITUAL, user_id, state.storage)
    params = {
        "ritual_id": ritual_id,
        "feedback": message.text
    }
    response = requests.post(url=get_base_url(router="sos_feedback"), params=params, headers=get_headers(user_id))
